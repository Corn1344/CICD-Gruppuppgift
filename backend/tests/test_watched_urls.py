...
def test_get_watched_urls_err(test_client):
    """This is a mock test that send a get request to /watched-urls/0, 
    should return error status_code = 404"""
    response = test_client.get('/watched-urls/0')
    assert response.status_code == 404


def test_get_watched_urls(test_client):
    ...
    with app.app_context():
        response = test_client.get("/watched-urls/1")
